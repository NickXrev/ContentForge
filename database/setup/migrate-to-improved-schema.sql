...
-- This handles the existing content_documents table

-- First, let's see what we have and migrate existing data
-- We'll add the new columns to the existing content_documents table

-- First create the new tables, then add foreign key references
...
ADD COLUMN IF NOT EXISTS parent_document_id UUID REFERENCES public.content_documents(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS assigned_to UUID REFERENCES public.users(id) ON DELETE SET NULL;

-- Update existing records to have proper platform and status
UPDATE public.content_documents 
SET platform = 'linkedin', status = 'draft' 
WHERE platform IS NULL;

-- Create the new tables that don't exist yet
...
-- Create RLS policies for new tables (drop first if they exist)
...
-- Insert some default categories for each team
